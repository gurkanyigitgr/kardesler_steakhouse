...
const menus = ['Dönerler', 'Ana Yemekler', 'Yan Lezzetler', 'Kahvaltılıklar'];
...
export default function MenuSelection() {
  const [menu, setMenu] = useState('Menü Seç');
  return (
    <Menu as='div' className='w-full h-full flex xl:flex-row'>
      <div className='relative flex-1'>
          ...
          <div className='w-full h-16 xl:h-full flex justify-center xl:justify-start xl:border-r xl:border-blackColor/10'>
            <div className='flex flex-col justify-center'>
              <div className='flex flex-col xl:flex-row items-center xl:gap-x-2 gap-y-2 xl:gap-y-0'>
                <MdRestaurantMenu className='text-accent' />
        ...
        <Transition {...transitionClasses}>
          <Menu.Items className='dropdown-menu shadow-lg absolute -top-64 xl:top-[30px] left-1/2 xl:left-0 z-10 transform -translate-x-1/2 xl:-translate-x-0 text-sm text-center xl:text-left w-full bg-whiteColor max-w-[332px] py-6 rounded-[10px]'>
            {menus.map((menu, i) => (
